...
export function getOTPEmailHtml({ otp }: OTPEmailParams): string {
  ...
  const digitBoxes = digits
    .map(
      (d) =>
        `<td style="width:40px;height:48px;text-align:center;font-size:24px;font-weight:bold;color:#1a1a1a;background:#f5f4f1;border:1px solid #e0ddd8;border-radius:8px;">${d}</td>`,
    )
